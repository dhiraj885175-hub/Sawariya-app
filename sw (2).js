...
//
// SPLASH/BOOT-SPEED FIX (2026-08): index.html mein splash-stuck bug fix
// kiya gaya hai (Firebase/OneSignal ab defer ki jagah dynamically, non-
// blocking load hote hain). Purane already-installed devices tak ye fix
// pahunchane ke liye version bump kiya hai — isse activate() ka purana-
// cache-cleanup turant chalega aur naya shell fetch hoga.
// SLOW-NETWORK RELIABILITY FIX (2026-08): do cheezein fix ki hain --
// (1) shell fetch ka 5-second timeout Motipur/Bihar jaisi genuinely slow
//     (2G/weak-4G) connections ke liye bahut tight tha -- network abhi
//     working thi, bas thodi slow, lekin 5s cross hote hi SW use turant
//     "offline" maan kar cache/fallback par bhej deta tha. Ab thoda zyada
//     time deते hain (neeche NETWORK_TIMEOUT_MS dekho) taaki genuinely
//     slow (par working) connection par bhi asli fresh page mil sake,
//     bina waqt se pehle hi haar maane.
// (2) activate() pehle HAMESHA purane-version caches turant delete kar
//     deta tha, chahe naye version ka shell cache install() mein safal
//     hua ho ya nahi (Promise.allSettled silently fail ho sakta tha agar
//     install hi kisi slow/flaky network ke dauraan hua ho). Agar naya
//     shell cache nahi ban paya AUR purana turant delete ho gaya, to agli
//     baar app khulne par SW ke paas koi bhi kaam ka cached shell nahi
//     bachta -- slow/flaky net par phir dobara fail hone par seedha
//     "offline" error mil jaata. Ab activate() pehle check karta hai ki
//     naya shell cache mein hai bhi ya nahi -- tabhi purane version clean
//     karta hai, warna agli baar phir try karta hai.
const SW_VERSION   = 'v7';
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.open(SHELL_CACHE).then((shellCache) =>
      shellCache.match('./index.html').then((newShellReady) => {
        if (!newShellReady) {
          // Naya shell abhi cache mein successfully save nahi ho paaya
          // (jaise install() ke waqt network flaky/slow tha) -- purane
          // version ke caches ko abhi mat hatao, warna agli baar phir
          // fail hone par app ke paas dikhane ko kuch nahi bachega. Agli
          // baar SW dobara activate try karega jab tak naya shell na aa
          // jaaye.
...
// Runtime assets (Firebase/OneSignal SDK, fonts, images) ko thoda zyada
// waqt dete hain kyunki inka apna koi cached "shell" fallback nahi hota,
// but ANR na ho isliye ye bhi hamesha bounded hai, kabhi infinite nahi.
const RUNTIME_TIMEOUT_MS = 10000;

// ABORT FIX: pehle sirf Promise race hoti thi -- asli fetch() cancel nahi
// hota tha, connection slot busy rehta tha aur slow network par baaki
// requests (shell navigation samet) bhi uski wajah se queue mein atak
// sakti thi. Ab AbortController se timeout hone par asli network request
// bhi cancel kar dete hain -- connection turant free ho jaata hai.
function fetchWithTimeout(request, ms, opts) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), ms);
  // request agar already ek Request object hai (jisme apna signal ho
  // sakta hai), usse clone karke apna signal jod dete hain.
  const reqInit = Object.assign({ signal: controller.signal }, opts || {});
  const signaledReq = (request instanceof Request)
    ? new Request(request, reqInit)
    : request;
  return fetch(signaledReq).finally(() => clearTimeout(timer));
}

function networkFirst(request, cacheName) {
  return caches.open(cacheName).then((cache) => {
    // HTTP-CACHE BYPASS FIX: sirf Service Worker cache hi nahi, browser ka
    // apna HTTP (disk) cache bhi GitHub Pages ke response headers ke
    // hisaab se is file ko cache kar sakta tha — matlab "network-first"
    // fetch() bhi kabhi-kabhi network ki jagah HTTP-cache se hi purana
    // jawab de deta tha, chahe GitHub par naya index.html upload ho chuka
    // ho. `cache:'no-store'` se ye fetch hamesha asli network se jaata
    // hai, HTTP cache poori tarah bypass ho jaata hai.
    return fetchWithTimeout(request, NETWORK_TIMEOUT_MS, { cache: 'no-store' })
...
// ANR FIX (biggest one): pehle is function mein KOI timeout nahi tha.
// Firebase/OneSignal SDK jaisi cheezein isi function se load hoti hain,
// aur app boot inhi <script defer> par depend karta hai. Pehli baar
// (khaali cache) + bahut slow network par ye fetch() hamesha ke liye
// latak sakta tha -- na resolve, na reject -- jiske wajah se poora app
// boot atak jaata tha aur WebView "isn't responding" dikhata tha. Ab
// isko bhi fetchWithTimeout se bound kar diya (RUNTIME_TIMEOUT_MS) taaki
// ye kabhi infinite wait na kare.
function staleWhileRevalidate(request, cacheName) {
  return caches.open(cacheName).then((cache) => {
    return cache.match(request).then((cached) => {
      const networkFetch = fetchWithTimeout(request, RUNTIME_TIMEOUT_MS)
        .then((networkResponse) => {
          // BUG FIX (Similar Products images kabhi cache hi nahi ho rahi
          // thi): i.ibb.co jaisi cross-origin images <img> tag se bina
          // `crossorigin` attribute ke load hoti hain, isliye browser
          // inhe "no-cors" mode mein fetch karta hai -- jawab mein SW ko
          // hamesha "opaque" response milta hai (status hamesha 0, chahe
          // asal mein request successful hi kyun na ho). Pehle sirf
          // `status === 200` wale response cache hote the, isliye ye
          // opaque image responses kabhi cache mein save nahi hote the --
          // matlab SW in par kaam hi nahi kar raha tha, har baar seedha
          // network se jaati thi (isliye slow/flaky net par purani/blank
          // dikhti thi). Ab `type === 'opaque'` ko bhi valid maan kar
          // cache karte hain -- basic (same-origin/cors) responses ke
          // liye status check waisa hi rehta hai, sirf opaque cross-origin
          // ke liye extra allow kiya hai.
        ...
        .catch((err) => {
          // Network fail/slow/timeout — purana cached jawab hi sahi.
          if (cached) return cached;
          // Cache bhi khaali (bilkul pehli visit + slow network) — ab
          // hamesha ke liye latakne ke bajaye saaf reject karo taaki
          // caller (script tag / img tag) apna normal error/retry
          // behaviour turant dikha sake, boot atka na rahe.
          throw err;
        });
      // Agar cache mein pehle se kuch hai, turant WAHI return karo (instant!),
      // network update chup-chaap background mein chalta rahega.
      // Agar cache khaali hai (pehli baar), tab hi network ka wait karna
      // padega — but ab wo wait bhi bounded hai (RUNTIME_TIMEOUT_MS).
